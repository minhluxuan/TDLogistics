...
class OrderValidation {
    ...
    validateCreatingOrder = (data) => {
        const schema = Joi.object({
            ...
            address_source: Joi.string().required(),
            address_dest: Joi.string().required(),
            ...
            express_type: Joi.number().min(0)
        }).strict();
        
        return schema.validate(data);
    }
    
    validateFindingOrder = (data) => {
        const schema = Joi.object({
            order_id: Joi.string(),
    ...
    validateCalculatingFee = (data) => {
        const schema = Joi.object({
            user_id: Joi.string().pattern(new RegExp(process.env.REGEX_USER_ID)),
            name_sender: Joi.string().pattern(new RegExp(process.env.REGEX_NAME)),
            phone_sender: Joi.string().pattern(new RegExp(process.env.REGEX_PHONE_NUMBER)),
